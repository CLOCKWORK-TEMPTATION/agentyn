...
/**
 * تكامل Google ADK مع الوكيل المتقدم - نسخة آمنة ومكتملة
...
async function testCalculatorSecurity() {
  ...
  for (const input of maliciousInputs) {
    try {
      ...
      console.error(`❌ خطأ في دفق الاستجابة: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
...
async function main() {
  try {
    ...
    // اختبارات متنوعة
    const testQueries = [
      { query: "ابحث عن آخر أخبار الذكاء الاصطناعي", sessionId: "test_1" },
      { query: "ما هو 5 + 5؟", sessionId: "test_2" },
      { query: "اكتب كود Python لحساب الأعداد الأولية", sessionId: "test_3" }
    ];
